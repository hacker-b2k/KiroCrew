...
class TestDocumentPass:
    ...
    @pytest.mark.asyncio
    async def test_one_unextractable_page_costs_only_itself(self, tmp_path, monkeypatch):
        """A page that raises costs its own page and nothing else.

        The blast radius is why this is pinned rather than left to the outer
        handler: that handler returns (), which loses the pages already read AND
        every later page, and the empty result is then cached under the content
        digest -- so a single bad page would empty the file for the life of the
        process, not merely for the request that met it."""
        pytest.importorskip("pdfplumber")
        import pdfplumber

        root = tmp_path / "pdf-partial"
        root.mkdir()
        path = root / "paper.pdf"
        _write_pdf(path, ["page one WIDGET", "page two", "page three GADGET"])
        f._GREP_DOC_CACHE.clear()

        real_open = pdfplumber.open

        def flaky_open(stream, *a, **kw):
            pdf = real_open(stream, *a, **kw)
            pages = list(pdf.pages)
            if len(pages) >= 2:

                def boom():
                    raise RuntimeError("this page is not extractable")

                pages[1].extract_text = boom  # type: ignore[method-assign]
            return pdf

        monkeypatch.setattr(pdfplumber, "open", flaky_open)

        # The page BEFORE the bad one and the page AFTER it are both still found.
        assert _files(await _grep(root, "widget")) == {"paper.pdf"}
        assert _files(await _grep(root, "gadget")) == {"paper.pdf"}
    ...
    def test_a_workbook_with_a_bloated_inventory_is_refused_before_openpyxl(
        ...
        assert f._grep_xlsx_segments(path.read_bytes(), str(path), f.time.monotonic() + 30) == (
            (),
            True,
        )
    ...
    def test_the_cache_evicts_least_recently_used(self):
        ...
        for i in range(f._GREP_DOC_CACHE_ENTRIES + 5):
            f._grep_doc_cache_put((".docx", str(i).encode()), (("doc", "x"),))
...
class TestDocumentExclusionIsCaseInsensitive:
    ...
    The document pass owns `.docx`/`.pptx`/`.pdf`/`.xlsx` and matches them by
    `splitext(name)[1].lower()`, so it claims `REPORT.PDF`. ripgrep's globs are
    case-SENSITIVE, so `!**/*.pdf` did not exclude that spelling -- rg read it as
...
class TestANewlineQueryIsAnsweredEmpty:
    ...
    async def test_trailing_whitespace_is_still_only_trimmed(self, tmp_path):
        ...
        assert {r["file"].split("/")[-1] for r in payload["results"]} == {"a.py"}


class TestAParseIsBoundedByTheDeadline:
    """The character cap bounds TEXT, not work.

    A workbook of millions of empty rows, or a PDF of many blank pages, yields no
    text -- so `_GREP_DOC_MAX_CHARS` never trips and the loop runs to the end of
    the document however long that takes. The extractors watch the shared deadline
    for exactly the rows and pages the cap cannot see.
    """
    ...
    def test_a_workbook_of_empty_rows_stops_at_the_deadline(self, tmp_path, monkeypatch):
        ...
        segments, complete = f._grep_xlsx_segments(
    ...
    async def test_a_deadline_truncated_parse_is_not_cached(self, tmp_path, monkeypatch):
        ...
        def cut_short(_data, _path, _ext, _deadline):
            return ((("", "partial WIDGET text"),), False)
