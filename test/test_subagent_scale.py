...
class TestDurableQueueScale:
    ...
    async def test_2000_spawns_are_2000_rows_and_at_most_64_dicts(self):
        ...
        with patch("kiro_crew.subagent.Stats"), patch("kiro_crew.subagent.sel"), \
                patch.object(SubagentManager, "_run", new=AsyncMock()):
            ids = [mgr.spawn(f"task {i}", parent_session_key="dashboard:s1").id for i in range(2000)]
    ...
    async def test_queue_survives_manager_loss_and_drains_fifo(self):
        ...
        first = SubagentManager(sessions=_mock_sessions(), ctx_builder=_mock_ctx(), max_concurrent=1)
        ...
        first._taskq.close()
        del first
        second = SubagentManager(sessions=_mock_sessions(), ctx_builder=_mock_ctx(), max_concurrent=3)
