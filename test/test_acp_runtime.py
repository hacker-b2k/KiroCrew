...
def kas_readiness_wire(monkeypatch, tmp_path):
    ...
    def status(
        ...
        meta = {"kiro": {"resource": {"source": {"origin": origin}}}}
        _feed(
            reader,
            {
                ...
                "params": {
                    "sessionId": sid,
                    "servers": [
                        {"name": "kirocrew-core", "status": state, "_meta": meta, **extra},
                        {"name": "kirocrew-dashboard", "status": "connected", "_meta": meta},
                        {"name": "external", "status": "failed", "errorMessage": "irrelevant"},
                        *(
                            {"name": name, "status": "connected", "_meta": meta}
    ...
    async def handshake(
        ...
        *,
        ...
        assert (
            request["params"]["_meta"]["kiro"]["customAgents"]
            == rt._kas_custom_agents.return_value.custom_agents
        )
        if pre_ready:
            status("connected")
...
@pytest.mark.parametrize(
    "state", ["failed", "disabled", "authorization", "timeout", "unreported", "missing-catalog"]
)
async def test_kas_readiness_refuses_failure_or_missing_report(kas_readiness_wire, resume, state):
    ...
    try:
        ...
        with pytest.raises(expected, match="kirocrew-core"):
            await asyncio.wait_for(start, 3.0)
        assert (
            wire.sent.empty()
        ), "A failed startup must neither prompt nor delete the saved session"
