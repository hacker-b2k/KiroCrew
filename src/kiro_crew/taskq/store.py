...
# three classes of on-loop take are tracked here, and an un-offloaded READ is not
...
# comment. The first two tracked classes are ORDERING problems; the third is a
# write whose RETURN VALUE is load-bearing:
#
...
# 3. The DRAINED spawn's ``taskq_defer``, in ``admission.gate``'s ``_deferred``
#    reached from the pump's ``_dispatch_async_impl``: its boolean is what tells
#    a durable row apart from a legacy in-memory ``_queue`` entry that never
#    reached the store, and ``_from_queue`` alone does not prove a row. Posting
#    it would refuse the legacy entry instead of deferring the durable one
#    (``test_queued_nonbatch_rejection_announced_exactly_once`` pins that).
#    It is the one un-offloaded write, and it is on the pressure path; closing
#    it needs a window-sourced signal threaded to the gate, not an offload.
#
...
class TaskStore:
    ...
    def cancel(self, task_id: str, *, reason: str = "") -> str | None:
        ...
        with self._lock:
            ...
            try:
                ...
                conn.execute(
                    ...
                    f"WHERE id=? AND state NOT IN {_SQL_TERMINAL}",
