...
from .types import FairnessSettings, tombstone_terminal_state
...
class _TaskqBridgeMixin(ManagerComponent):
    ...
    def taskq_cancel_queued(self, agent_id: str) -> dict[str, Any] | None:
        ...
        Race-safe against the drain by construction: ``cancel`` is one UPDATE
        that only touches a non-terminal row, and ``claim`` only touches a
        claimable one, so whichever lands second sees the other's write. A row
        already claimed (``admitted``) is cancelled in the store too -- the
        spawn re-reads the state before it registers and stops there.
        """
        from kiro_crew import taskq as _taskq
        ...
        try:
            ...
            if rec is None or rec.terminal:
                return None
            if rec.state not in _taskq.CLAIMABLE and rec.state != _taskq.ADMITTED:
                return None
            previous = store.cancel(agent_id, reason="user_stop")
        except _taskq.TaskStoreUnavailable:
