...
class _GateMixin(ManagerComponent):
    ...
    def spawn_impl(
        ...
        def _deferred(reason: str) -> SubagentInfo | None:
            ...
            # A row this very call just wrote (``_store_accepted``) posts its
            # defer to the writer thread instead of writing on the loop: the
            # row is queued either way, the defer only sets when the pump
            # looks again. A drained entry keeps the inline write, whose
            # False answer is what tells a legacy in-memory entry apart --
            # ``_from_queue`` alone does not prove a row exists, because
            # ``_queue`` also holds entries that never reached the store.
            if _store_accepted:
        ...
        if not mem_ok:
            ...
            deferred = (
                _deferred(f"low memory: {avail_gb:.1f} GB available, need {min_mem:.0f} GB")
                if _durable
                else None
            )
        ...
        if not admission.admitted and not _dispatch_now:
            ...
            deferred = _deferred(str(admission.reason)) if _durable else None
            if deferred is not None:
                return deferred
            info = SubagentInfo(
                id=agent_id,
