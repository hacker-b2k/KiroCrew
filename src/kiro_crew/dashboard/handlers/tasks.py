...
* ``POST /api/tasks/{task_id}`` -- an action on a PARKED runner row:
  ...
  orphan the route below exists to avoid.
...
#: States in which a row's RUNTIME is live with no wait parked under it: the
#: step, agent call or run is executing, so there is nothing for a wait-level
#: lever to end and only the owning run can stop it. ``admitted`` is not one --
#: it is claimed but never started, the state the reconciler requeues.
_LIVE_RUNTIME_STATES = frozenset({taskq_model.STARTING, taskq_model.RUNNING})
...
async def _runner_cancel(state: Any, record: Any) -> bool | None:
    ...
    if record.state not in _LIVE_RUNTIME_STATES:
        if admission is None:
            return False
        return bool(
            await asyncio.to_thread(
                admission.cancel_wait, task_id, reason="cancelled from /api/tasks"
            )
        )
...
async def api_task_action(request: web.Request) -> web.Response:
    ...
    if action == "answer_input":
        ...
        if record.state in _LIVE_RUNTIME_STATES:
            return web.json_response(
                {"ok": False, "code": "not_waiting", "task": row_payload(record)}, status=409
            )
        ok = bool(
            await asyncio.to_thread(
                admission.cancel_wait, task_id, reason="wait cancelled from /api/tasks"
            )
        )
    try:
        after = await asyncio.to_thread(store.get, task_id)
    ...
    if not ok:
        return web.json_response(
            {"ok": False, "code": "action_failed", "action": action, "task": payload}, status=409
        )
