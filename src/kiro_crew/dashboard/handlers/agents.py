...
async def _create_crew(
    ...
    *,
    ...
    publication, before the row lands: a record with no row is inert, whereas a
    row with no record is a session agent the owner never hired. A failure in
    either half unwinds the other, so the two never disagree.
