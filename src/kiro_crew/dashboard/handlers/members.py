...
def enrolled_member_ids(cfg: KiroCrewConfig, *, strict: bool = False) -> list[str]:
    ...
    logs and answers no members, which fails closed for the roster.
    """
    try:
...
async def api_members(request: web.Request) -> web.Response:
    ...
    # agent, not a crewmate.
    roster_ids = await asyncio.to_thread(enrolled_member_ids, cfg)
    rows: list[dict] = []
...
async def _fire_member(
    ...
    if slugmates:
        return web.json_response(
            {
                "error": (
                    f"Nothing was fired. Another member, '{slugmates[0]}', has the same "
                    f"short name ('{slug}') as '{display_name}', so they share files. "
                    "Rename or fire that member first."
                ),
                "code": "slug_collision",
            },
            status=409,
    ...
    async with _agents_handlers._get_config_lock():
        ...
        def _space_still_ours(agents_doc: dict) -> str | None:
            ...
            if others:
                ...
                return (
                    f"Nothing was fired. Another member, '{others[0]}', has the same "
                    f"short name ('{slug}') as '{display_name}', so they share files. "
                    "Rename or fire that member first."
                )
            return None
