...
**New crew** stays a plain create (bind to a shared template) and enrolls nobody
...
- **The predicate** (`handlers/members.py::enrolled_member_ids`): a row is a
  ...
  enrolled; a record with no row is inert. Reads
  are lenient for the roster (unreadable sidecar -> no members, logged) and
  STRICT for routes that write for a member (the DM thread open, the rules
  write: 503 `members_unavailable` rather than "not a member").
- **Who reads it.** `GET /api/members` lists enrolled ids only;
  ...
  **Agent templates** (picking a template starts a session and enrolls nobody;
  picking a crewmate references the existing identity). The memory page offers
  **Open crewmate conversation** only when the private store's owner is on the
  ...
  session agent with no thread to open. Sessions, crons and dispatch bindings
...
- **Upgrade: nothing is enrolled.** No build before this one wrote a mark that
  ...
  re-keyed with the row (`rename_crewmate_record`).
...
slug with another LIVE member is refused (409 `slug_collision`, in plain words -- "Nothing was fired. Another member, 'X', has the same short name ('slug') as 'Y', so they share files. Rename or fire that member first." -- never the internal "member space" / "shorten to" vocabulary): `members/<slug>/`,
...
`test_a_legacy_app_row_is_attributed_once_when_one_app_ships_the_name`). A local
...
(Escape cancels, an unchanged name writes nothing). **Same-label twins wear their
...
beside the label, on those rows only, so the destination says which of the two it
is the way the picker did; distinct labels stay a list of names (pinned:
`MembersPage.identity.test.tsx`). The editor is
