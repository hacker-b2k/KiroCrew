...
`splitext(name)[1].lower()`, so a case-sensitive exclusion leaves `REPORT.PDF` in
...
extracts `.docx`/`.pptx`/`.pdf`/`.xlsx`. The character cap bounds TEXT, not work:
a workbook of empty rows or a PDF of blank pages produces none, so the worksheet
row loop samples the deadline every `_GREP_ROW_DEADLINE_STRIDE` rows and the page
loop checks it per page. Extraction is cached by a digest of the bytes parsed,
