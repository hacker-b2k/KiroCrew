...
describe('RemoteCrewPanel', () => {
  ...
  it('renames a configured crew and refreshes its visible label', async () => {
    ...
    const form = within(await screen.findByRole('group', { name: /Rename dev-box-1/i }))
    ...
    // The rest of the record sits behind a collapsed disclosure, so the
    // one-field task reads as one field with everything else reachable.
    expect(form.getByText(/Other settings \(read-only\)/)).toBeInTheDocument()
    // The row menu's focus trap is still mounted when the form commits and the
    // menu restores focus to its trigger as it closes, so the caret has to land
    // in Name AFTER both — a mount-time autofocus is pulled back into the menu.
