...
/** A stable empty default for `retiredCue`, so the prop's identity does not
 *  change on every render the way a literal `[]` would. */
const EMPTY_RETIRED_CUE: readonly AvatarState[] = []
...
export default function CrewAvatarBuilder({
   ...
   * whichever the reader cares about most.
   */
  retiredCue?: readonly AvatarState[]
  /**
  ...
  const retiredCueLabel = retiredCue.map(state => t(STATE_LABEL_KEYS[state])).join(', ')
