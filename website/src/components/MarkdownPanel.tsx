...
import { readFile, type FileRead } from '../utils/fileReadQuery'
...
export default memo(forwardRef<MarkdownPanelHandle, Props>(function MarkdownPanel({ filePath, content, binary, onContentChange, onDiskContent, onSave, onClose, liveWatch, onSubmitComments, connected = true, onRefresh, reserveWidth, initialDiffMode, onDiffModeChange, embedded, active = true, savedBaseline, revealLine, onRevealConsumed, browserRail, railOpen, onRailToggle, scrollMemoryKey }: Props, ref) {
   ...
   *
   * Ticket ordering lives in `readFile`, so Refresh, Cancel, the file watch,
   * chip clicks and cold-tab hydration all share one latest read per path.
   * Reporting stays with each caller because only it knows whether a human is
  ...
  const readFromDisk = useCallback(async (): Promise<DiskRead> => {
    ...
    return readFile(filePath, ac.signal)
  }, [filePath])

  // Withdrawing this panel's read turns it into `superseded` inside `readFile`,
  // so a path change or unmount cannot apply or report its result.
  useEffect(() => () => {
    diskReadAbortRef.current?.abort()
    diskReadAbortRef.current = null
  }, [filePath])

  /** Apply the winning disk read to the tab. The read arbiter owns ordering;
   *  there is no file-read cache to mirror. */
