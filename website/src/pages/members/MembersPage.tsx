              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className={`flex min-w-0 items-baseline ${ROW_TITLE_CLS} font-semibold text-text`} data-testid="member-row-label">
                    ...
                    {twinIds.has(m.name) && (
                      <span className="ml-1.5 shrink-0 truncate max-w-[40%] rounded border border-border bg-bg-elevated px-1 font-mono text-[11px] font-normal text-muted" title={t('pages.membersPage.member_id')} aria-label={`${t('pages.membersPage.member_id')}: ${m.name}`} data-testid="member-row-id">{m.name}</span>
                    )}
                    {m.role && (
