...
describe('HireGalleryPage', () => {
  ...
  it('lists cards from all sources with role, duty, three tags and the actions the crewmate count calls for', async () => {
    ...
    expect(within(off).getByTestId('hire-card-unavailable')).toHaveTextContent('ghost hasn’t finished setting up this agent. Use “Browse apps” above, turn ghost off and on again, then come back.')
    expect(within(off).getByTestId('hire-button')).toHaveAttribute('title', 'ghost hasn’t finished setting up this agent. Use “Browse apps” above, turn ghost off and on again, then come back.')
